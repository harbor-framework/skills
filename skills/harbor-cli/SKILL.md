...
| Kimi CLI | `kimi-cli` | Model format: `provider/model` (e.g., `kimi/kimi-k2`, `openrouter/moonshotai/kimi-k2.6`). Supported providers: `kimi`, `moonshot`, `openai`, `anthropic`, `gemini`, `openrouter`. Needs matching API key (`KIMI_API_KEY`, `MOONSHOT_API_KEY`, `OPENROUTER_API_KEY`, etc.) |
...
**Model name format varies by agent.** Claude Code uses `anthropic/claude-sonnet-4-1`, Cline CLI uses `provider:model-id` (e.g., `openrouter:anthropic/claude-opus-4.5`), Goose uses `provider/model_name`, Kimi CLI uses `provider/model` (e.g., `kimi/kimi-k2` or `openrouter/moonshotai/kimi-k2.6`). A `ValueError` about model format usually means the wrong format for that agent.
